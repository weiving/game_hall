  ...
  <div id="updateFundsPwd" class="page-wrap">
    ...
    <div class="page-content">
      <div class="set-body">
        ...
        <div class="row last">
          <input type="password" class="again-password" v-model="againPwd" placeholder="再次输入新资金密码">
        </div>
    ...
    methods: {
      ...
      update() {
        if (this.oldPwd == '' || this.oldPwd == undefined) {
          ...
          this.$http
            .post(`/api/v1/userdata/w/reset_funds_password/${user_id}/${username}?session=${session}`, params)
            .then(res => {
